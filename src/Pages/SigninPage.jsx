...
function SigninPage() {
  ...
  return (
    <div className="flex items-center justify-center w-screen h-screen bg-violet-800 ">
      {mutation.isPending && <Loader/>} 
      <form onSubmit={handleSubmit} className=" w-full sm:w-[450px] rounded-md   bg-white p-4 gap-4 flex flex-col m-1">
      <h4 className="text-gray-900 text-lg self-center font-semibold">Sign In</h4>
      {error?.message && <span className='text-red-800 text-[14px] items-center self-center flex gap-1 mt-1'> <AiFillWarning size={18} className='flex-shrink-0'/> {error.message}</span>}
          ...
          <div>    
            ...
            <div className="border h-[45px] rounded-md  flex items-center bg-white gap-1 p-2">
              ...
              <input
                className="border-none bg-white outline-none placeholder:text-[13px] text-xs p-2 placeholder:text-zink-400 text-gray-500  w-full"
          ...
          <div className="flex gap-2">
            <p className="text-[13px] text-gray-700"> Don&apos;t have an Account?</p> <Link to='/sign-up' className='text-[13px] font-bold text-violet-900' >Sign Up </Link >
